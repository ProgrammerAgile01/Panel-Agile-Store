...
export function FeatureProductSplit() {
  ...
  const handleExport = () => {
    ...
    if (activeTab === "menus") {
      ...
      for (const p of filteredFeatures.global) pushFeat("GLOBAL", p);
      for (const [, arr] of Object.entries(filteredFeatures.modules))
        for (const p of arr) pushFeat(p.module_name || "Module", p);
    }
    const blob = new Blob([rows.join("\n")], {
      type: "text/csv;charset=utf-8",
    });
  ...
  const submitEditPrice = async () => {
    ...
    try {
      ...
      setEditOpen(false);
      setEditTarget(null);
      await handleLoadAPI(false, false);
    } catch (e: any) {
      toast.error(e?.message || "Failed to update price.");
    } finally {
      setIsSaving(false);
                          ...
                          filteredFeatures.global.map((feature) => (
                            <div key={feature.id} className="space-y-2">
                                ...
                                <CardContent className="p-4">
                                  <div className="flex gap-3">
                                    <div className="min-w-0 flex-1">
                                      <div className="font-semibold text-base whitespace-normal break-words line-clamp-2">
                                        {feature.name}
                                      </div>
                                      <div className="text-sm text-muted-foreground mt-1 whitespace-normal break-words">
                                        {feature.description}
                                      </div>
                                    </div>
                                  </div>
                              ...
                              {feature.children?.map((child) => (
                                <Card
                                  key={child.id}
                                  className="rounded-lg bg-muted/30 ml-3"
                                >
                                  <CardContent className="p-4">
                                    ...
                                    <div className="mt-2 flex items-center gap-2">
                                      ...
                                      <Button
                                        ...
                                        className="font-mono text-xs hover:bg-muted ms-auto"
                                        aria-label={`Copy feature ID ${child.id}`}
                                      >
                                        {child.id}
                                        <Copy className="h-3 w-3 ml-1" />
