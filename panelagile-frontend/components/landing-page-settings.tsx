...
function buildSectionsPayload(
  sectionsIdRef: React.MutableRefObject<LandingPageSectionUI[]>,
  sectionsEnRef: React.MutableRefObject<LandingPageSectionUI[]>,
  themes: Record<string, ColorTheme> | null | undefined,
  defaultTheme: ColorTheme
) {
  const idList = sectionsIdRef.current || [];
  return idList.map((sID) => {
    const sEN =
      (sectionsEnRef.current || []).find((x) => x.id === sID.id) || null;
    const themeForThis =
      themes && themes[sID.id]
        ? themes[sID.id]
        : sID.content?.theme || sEN?.content?.theme || defaultTheme;
    return {
      id: sID._recordId || undefined,
      section_key: sID.id,
      name: sID.name,
      enabled: !!sID.enabled,
      display_order: Number(sID.order ?? 0),
      content: { ...(sID?.content ?? {}), theme: themeForThis }, // ID -> content
      content_en: { ...(sEN?.content ?? {}), theme: themeForThis }, // EN -> content_en
    };
  });
}
...
const DEFAULT_SECTIONS: LandingPageSectionUI[] = [
  {
    ...
    content: {
      title: "See Product in Action",
      subtitle:
        "Watch how Product transforms your rental property management with intuitive interfaces && powerful features.",
      ctaText: "Start Free Trial",
      backgroundImage:
        "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/image-7Q8YkkhfFWdiP2ur6esDmeobrRv2yg.png",
      videoUrl: "",
  ...
  },
  {
    ...
    content: {
      companyName: "Agile Store",
      description:
        "Empowering businesses with digital solutions that drive growth && efficiency. Transform your operations with our suite of professional tools.",
      sections: [
        { title: "PRODUCT", links: ["Features", "Pricing", "Demo", "Reviews"] },
        {
          title: "SUPPORT",
  ...
  },
  {
    ...
    content: {
      ...
      faqs: [
        ...
        {
          question: "Can I customize the booking forms && customer portal?",
          answer:
            "Yes, you can fully customize booking forms, customer portals, and branding to match your business needs.",
        },
        {
        ...
        },
        {
          question: "Is there a mobile app for managing rentals on the go?",
          answer:
            "Yes, we have native mobile apps for both iOS && Android that allow you to manage your rentals anywhere.",
        },
        {
          question: "What about a free plan?",
          answer:
            "We offer a free trial, but our paid plans start at $29/month to ensure we can provide the best service && support.",
        },
        {
          question: "Can I integrate this with my existing tools?",
          answer:
      ...
      contactSection: {
        title: "Still have questions?",
        subtitle:
          "Our support team is here to help you get started && find the right plan for you.",
        ctaText: "Get Demo",
        contactText: "Contact Support",
      },
    },
...
export function LandingPageSettings() {
  ...
  const [sections, setSections] =
    useState<LandingPageSectionUI[]>(DEFAULT_SECTIONS);
  // Keep EN/ID copies in refs, so UI keeps same setSections API
  const sectionsEnRef = useRef<LandingPageSectionUI[]>(DEFAULT_SECTIONS);
  const sectionsIdRef = useRef<LandingPageSectionUI[]>(DEFAULT_SECTIONS);
  const setSectionsBoth = useCallback(
    (updater: any) => {
      setSections((prev) => {
        const next = typeof updater === "function" ? updater(prev) : updater;
        if (lang === "en") sectionsEnRef.current = next;
        else sectionsIdRef.current = next;
        return next;
      });
    },
    [lang]
      ...
      try {
        ...
        if (landing.status === "fulfilled" && mounted) {
          ...
          if (apiSections.length) {
            ...
            const mappedEN = apiSections
              .map((s: any) => ({
                id: s.section_key,
                name: s.name,
                enabled: !!s.enabled,
                order: Number(s.display_order ?? 0),
                content: s.content_en ?? {},
                _recordId: s.id,
              }))
              .sort((a: any, b: any) => a.order - b.order);

            // Save both languages into refs; show current lang
            sectionsIdRef.current = mappedID;
            sectionsEnRef.current = mappedEN;
            setSections(lang === "en" ? mappedEN : mappedID);

            // Hydrate themes from content.theme per-section (prefer ID, fallback EN)
            const loadedThemes: Record<string, ColorTheme> = {};
            for (const sec of apiSections) {
              const t = (sec?.content?.theme ?? sec?.content_en?.theme) as any;
              if (t && typeof t === "object") {
                loadedThemes[sec.section_key] = { ...DEFAULT_THEME, ...t };
              }
            }
            setThemes(loadedThemes);

            // Ensure activeSection is valid
            const exists = (lang === "en" ? mappedEN : mappedID).some(
              (m) => m.id === activeSection
            );
            if (!exists && (mappedID.length || mappedEN.length)) {
              setActiveSection((mappedID[0] || mappedEN[0]).id);
            }
          } else {
            sectionsIdRef.current = DEFAULT_SECTIONS;
            sectionsEnRef.current = DEFAULT_SECTIONS;
            setSections(
              lang === "en" ? sectionsEnRef.current : sectionsIdRef.current
            );
            setThemes({});
            setActiveSection("hero");
          }
  ...
  const handleSaveV2 = async () => {
    ...
    setSaving(true);
    try {
      // Materialize uploads for BOTH languages
      const cleanEN = await materializeUploads(sectionsEnRef.current || []);
      const cleanID = await materializeUploads(
        sectionsIdRef.current || sections
      );

      // Build index maps
      const mapEn: Record<string, LandingPageSectionUI> = Object.fromEntries(
        (cleanEN || []).map((s) => [s.id, s])
      );
      const mapId: Record<string, LandingPageSectionUI> = Object.fromEntries(
        (cleanID || []).map((s) => [s.id, s])
      );
      ...
      // Build payload sections with both content + content_en
      const sectionsPayload = unionIds
        .map((secId) => {
          const sID = mapId[secId];
          const sEN = mapEn[secId];

          // theme priority: explicit state theme -> theme inside sID.content -> DEFAULT
          const themeForThis: ColorTheme =
            themes[secId] ??
            (sID && sID.content && (sID as any).content.theme
              ? { ...DEFAULT_THEME, ...(sID as any).content.theme }
              : DEFAULT_THEME);

          return {
            id: sID?._recordId,
            section_key: secId,
            name: (sID?.name || sEN?.name || secId) as string,
            enabled: Boolean(sID?.enabled ?? true),
            display_order: Number((sID?.order ?? sEN?.order ?? 0) as number),
            content: { ...(sID?.content ?? {}), theme: themeForThis },
            // FIX: ensure English goes to content_en and Indonesian goes to content
            content_en: { ...(sEN?.content ?? {}), theme: themeForThis },
          };
        })
      ...
      // ===== Save Pricelist (language-neutral) from ID side, fallback to EN =====
      const pricing =
        (cleanID || []).find((s) => s.id === "pricing") ||
        (cleanEN || []).find((s) => s.id === "pricing");

      if (pricing) {
        const currency = (pricing as any).content?.currency || "IDR";
        const tax_mode =
          ((pricing as any).content?.tax_mode as "inclusive" | "exclusive") ||
          "inclusive";
        const plans = Array.isArray((pricing as any).content?.plans)
          ? (pricing as any).content.plans
          : [];
      ...
      // ===== Refresh from server (to sync _recordId && both languages) =====
      const fresh = await getLandingByProduct(selectedProduct);
      const apiSections: any[] = Array.isArray(fresh.sections)
        ? fresh.sections
        : [];
      ...
      // Re-hydrate themes (prefer ID's theme, fallback to EN)
      const loadedThemes: Record<string, ColorTheme> = {};
      for (const sec of apiSections) {
        const t = (sec?.content?.theme ?? sec?.content_en?.theme) as any;
        if (t && typeof t === "object") {
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => {
                setLang("en");
                setSections(sectionsEnRef.current);
              }}
            >
            ...
            <Button
              ...
              onClick={() => {
                setLang("id");
                setSections(sectionsIdRef.current);
              }}
            >
            ...
            {lang === "id" && (
              <Button
                ...
                className="gap-2 bg-transparent"
                onClick={async () => {
                  // Translate ALL sections from EN -> ID using translateObjectStrings
                  const base = sectionsEnRef.current || [];
                  const translated = await Promise.all(
                    base.map(async (sec) => ({
                      ...sec,
                      content: await translateObjectStrings(sec.content, {
                        from: "en",
                        to: "id",
                      }),
                    }))
                  );
                  sectionsIdRef.current = translated;
            ...
            {lang === "id" && (
              <div className="w-full flex justify-end mb-2">
                <Button
                  ...
                  className="gap-2 bg-transparent"
                  onClick={async () => {
                    const baseSec = (sectionsEnRef.current || []).find(
                      (s) => s.id === activeSection
                    );
                    if (!baseSec) return;
                    const tr = await translateObjectStrings(baseSec.content, {
                      from: "en",
                      to: "id",
                    });
                    setSections((prev: any) =>
                      prev.map((s: any) =>
                        s.id === activeSection ? { ...s, content: tr } : s
                      )
                    );
                  }}
                >
                    ...
                    <div className="w-full flex justify-end mb-2">
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          if (!baseSec) return;
                          const tr = await translateObjectStrings(
                            baseSec.content,
                            { from: "en", to: "id" }
                          );
                          setSections((prev: any) =>
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                const translated = await Promise.all(
                                  base.map(async (sec) => ({
                                    ...sec,
                                    content: await translateObjectStrings(
                                      sec.content,
                                      {
                                        from: "en",
                                        to: "id",
                                        ...
                                        {lang === "id" && (
                                          <Button
                                            ...
                                            onClick={async () => {
                                              // Translate ALL sections from EN -> ID using translateObjectStrings
                                              const base =
                                                sectionsEnRef.current || [];
                                              const translated =
                                                await Promise.all(
                                                  base.map(async (sec) => ({
                                                    ...sec,
                                                    content:
                                                      await translateObjectStrings(
                                                        sec.content,
                                                        {
                                                          from: "en",
                                                          to: "id",
