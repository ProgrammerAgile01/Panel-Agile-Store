...
export function Durations() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      // Search is handled in filteredDurations
    }, 250);
  ...
  const filteredDurations = useMemo(() => {
    const filtered = durations.filter((duration) => {
      const matchesSearch =
        duration.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        duration.code?.toLowerCase().includes(searchQuery.toLowerCase()) ||
        "";
      const matchesStatus =
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredDurations.map((duration) => (
                  <tr key={duration.id} className="border-t hover:bg-muted/30">
                    <td className="p-3">
                      <Checkbox
                    ...
                    <td className="p-3">
                      <Badge
                        variant={
                          duration.status === "active" ? "default" : "secondary"
                        }
                      >
                        {duration.status}
                      </Badge>
